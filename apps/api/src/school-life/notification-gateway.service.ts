...
export type DeliveryStatus = "QUEUED" | "SENT_TO_PROVIDER" | "DELIVERED" | "FAILED" | "RETRYING" | "UNDELIVERABLE";
...
export class NotificationGatewayService {
  ...
  private async dispatchWithWebhook(
    ...
    const tokenKey = channel === "EMAIL" ? "NOTIFY_EMAIL_WEBHOOK_TOKEN" : "NOTIFY_SMS_WEBHOOK_TOKEN";
    ...
    try {
      ...
      return {
        provider: typeof parsed.provider === "string" && parsed.provider ? parsed.provider : `WEBHOOK_${channel}`,
        providerMessageId,
        deliveryStatus
      };
    } finally {
