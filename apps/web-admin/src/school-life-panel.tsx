...
export function SchoolLifePanel(props: SchoolLifePanelProps): JSX.Element {
  ...
  const loadTimetableReferences = useCallback(async () => {
    ...
    if (!roomsResponse.ok) {
      onError(await parseError(roomsResponse));
      return;
    }
    if (!assignmentsResponse.ok) {
      onError(await parseError(assignmentsResponse));
      return;
    }

    setRooms((await roomsResponse.json()) as RoomRef[]);
    setTeacherAssignments((await assignmentsResponse.json()) as TeacherAssignmentRef[]);
  }, [api, onError]);
