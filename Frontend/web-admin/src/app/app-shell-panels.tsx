...
export function FeatureUnavailableScreen(props: {
  ...
  return (
    <section className="panel table-panel" role="status">
      <p className="eyebrow">{t("Fonctionnalité désactivée")}</p>
      <h2>{t(props.featureLabel)}</h2>
      <p className="subtle">
        {t("Cette fonctionnalité n’est pas activée dans cet environnement.")}
      </p>
  ...
  return (
    <section key={`context-${props.tab}`} className="panel context-bar">
      <div className="context-copy">
        <p className="eyebrow">{t("Module actif")}</p>
        <h2>{t(props.activeLabel)}</h2>
        {props.isEnrollmentsContext ? (
          <p className="section-lead">{t("Gérez les inscriptions et placements académiques des élèves.")}</p>
        ) : null}
    ...
    <footer className="panel app-footer app-footer-minimal">
      <div className="footer-head">
        ...
        <div className="footer-meta">
          <span>{t("Année")} : {props.schoolYearLabel}</span>
          <span>{t("Dernière sync")} : {props.lastSyncLabel}</span>
          {props.apiConnectionStatus !== "online" ? <span>{t(props.apiStatusText)}</span> : null}
        </div>
      </div>
    </footer>
  );
